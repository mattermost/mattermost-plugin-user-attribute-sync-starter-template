...
- **Every handler starts with the sysadmin check.** `checkSysAadmin()` reads the `Mattermost-User-Id` header — the only header the plugin can trust, because the Mattermost server sets it on the way in and strips any client-supplied value — and requires `model.PermissionManageSystem`. There is no route-level middleware, so a new route that forgets the call is silently unauthenticated.
